...
class cl_tba_cards extends Entity_Object {
    ...
    /* CONSTANT */
    private static $t_name="tba_cards"; //  table name
    
    private static $t_qry_id="SELECT * FROM tba_cards WHERE id=:id";   
    private static $t_qry_owner="SELECT * FROM tba_cards WHERE tba_cc_site_id_generator=:id";   
    private static $t_qry_gnerator="SELECT * FROM tba_cards WHERE tba_cc_site_id_owner=:id";   
    
    private static $t_upd_status="UPDATE tba_cards SET status=:status WHERE id=:id";    
    private static $t_upd_amount="UPDATE tba_cards SET credits_amount=:credits_amount WHERE id=:id";    
    
    
    private static $t_qry_insert="INSERT INTO tba_cards (id, 
    ...
    */
    ...
    public function get($id) {
        ...
        # parameter:
        $stpdo = $this->mypdo->prepare(cl_tba_cards::t_qry_id);
        ...
        if($arrcode[0] == $this->transaction_ok){//query ok
            ...
            $this->toLog("Errore in scl_tba_cards - query: ".cl_tba_cards::t_qry_id." - param: ".$id);
    ...
    function set($var_into) {
        $errors=array();
        $stpdo = $this->mypdo->prepare(cl_tba_cards::t_qry_insert);
        
        # verify parameter:
        foreach(cl_tba_cards::$var_list as $id=>$val) {
        ...
        if($arrcode[0] == $this->transaction_ok){//query ok
            ...
            $this->toLog("Errore in cl_tba_cards - query: ".cl_tba_cards::t_qry_insert." - param: ".$var_into);
            return array('result'=>0);
        }
    }
    ...
    function set_credits_amountt($id, $credits_amount) {
        $errors=array();
        $stpdo = $this->mypdo->prepare(cl_tba_cards::t_upd_amount);
        ...
        if($arrcode[0] == $this->transaction_ok){//query ok
            ...
            $this->toLog("Errore in cl_tba_cards - query: ".cl_tba_cards::t_upd_amount." - param: ".$var_into);
            return array('result'=>0);
        }
    }
